# -*- coding: utf-8 -*-
import requests,threading,Queue,time,random,json
...
class CrawlThread(threading.Thread):
    ...
    def run(self):
        print '启动蜘蛛%d号' % self.num
        while self.task_q.qsize() > 0:#队列长度大于0
            url = self.task_q.get()
            print '%d号蜘蛛采集%s' % (self.num,url)
            time.sleep(random.randint(1,3))
            response = self.sess.get(url)
            html = response.text
            self.data_q.put(html)
        print '蜘蛛%d号结束工作' % self.num
# 解析线程
...
if __name__=='__main__':
    start = time.time()
    main()
    print 'resume',time.time() - start#1个线程，24秒完成任务，10个线程，7秒完成，5个线程10秒完成
